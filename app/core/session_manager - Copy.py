...
class SessionManager:
    ...
    def sanitize(self, session_id: str, text: str, findings: List[Dict[str, Any]]) -> str:
        ...
        for i, finding in enumerate(sorted_findings):
            # Create unique token
            token = f"__VM_TOKEN_{len(findings) - i - 1}__"
            
            # Extract the sensitive data
            start = finding["start"]
            end = finding["end"]
            original_value = finding["match"]
            
            # Replace in text (working backwards preserves positions)
    ...
    def restore(self, session_id: str, text: str) -> str:
        ...
        for token, data in mapping.items():
            restored = restored.replace(token, data["value"])
